...
if ($id <= 0 || $client_name === "" || !$date || !$time || $service_id <= 0 || $employee_id <= 0) {
...
$stmt = $conn->prepare(
    "UPDATE appointments SET client_name = ?, service_id = ?, employee_id = ?, date = ?, time = ? WHERE id = ?"
);
$stmt->bind_param("siissi", $client_name, $service_id, $employee_id, $date, $time, $id);
